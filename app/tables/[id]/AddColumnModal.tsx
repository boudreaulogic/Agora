...
export function AddColumnModal({ tableId }: { tableId: string }) {
  ...
  async function handleCreate() {
    ...
    if (selectedType === 'formula' && !formula.trim()) {
      alert('Formula is required for formula columns');
      return;
    }

    if (selectedType === 'linked_record' && !linkedTableId) {
      alert('Please select a table to link to');
      return;
    }

    if (selectedType === 'lookup' && !lookupLinkedColumnId) {
      alert('Please select a linked record column');
      return;
    }

    if (selectedType === 'lookup' && !lookupFieldId) {
      alert('Please select a field to display');
      return;
    }

    if (selectedType === 'rollup' && !rollupLinkedColumnId) {
      alert('Please select a linked record column');
      return;
    }

    if (selectedType === 'rollup' && !rollupFunction) {
      alert('Please select an aggregation function');
      return;
    }

    if (selectedType === 'rollup' && rollupFunction !== 'COUNT' && !rollupFieldId) {
      alert('Please select a field to aggregate');
      return;
    }
            ...
            <div className="p-6 overflow-y-auto flex-1">
              ...
              <div className="mb-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-0.5">Must have a value when creating rows</p>
                  </div>
                  <button
                    type="button"
                ...
                <div className="grid grid-cols-2 gap-3">
                  {COLUMN_TYPES.map((type) => (
                    <button
                      ...
                      className={`flex items-start p-3 border-2 rounded-lg text-left transition-all ${
              ...
              {selectedType === 'lookup' && (
                <div className="mb-6 space-y-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                      Step 1: Select Linked Record Column <span className="text-red-500">*</span>
                    </label>
                    {loadingLinkedColumns ? (
                      ...
                      <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                        <p className="text-sm text-yellow-800">
                          No linked record columns found. Create a Linked Record column first to use lookups.
                        </p>
                      </div>
                    ) : (
                      <select
                        value={lookupLinkedColumnId}
                        onChange={(e) => setLookupLinkedColumnId(e.target.value)}
                        className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                      >
                        <option value="">Select a linked record column...</option>
                        {linkedRecordColumns.map((col) => (
                          <option key={col.id} value={col.id}>
                            🔗 {col.name} → {col.linkedTable?.name || 'Unknown table'}
                          </option>
                        ))}
                      </select>
                    )}
                    <p className="mt-1 text-xs text-gray-500">
                      Choose which linked record relationship to follow
                    </p>
                  </div>
                  ...
                  {lookupLinkedColumnId && (
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">
                        Step 2: Select Field to Display <span className="text-red-500">*</span>
                      </label>
                      {loadingFieldColumns ? (
                      ...
                      <p className="mt-1 text-xs text-gray-500">
                        This field's value will be pulled from linked records
                      </p>
                    </div>
                  )}
                ...
                <div className="mb-6 space-y-4">
                  ...
                  <div>
                    ...
                    {loadingLinkedColumns ? (
                      ...
                      <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                        <p className="text-sm text-yellow-800">
                          No linked record columns found. Create a Linked Record column first to use rollups.
                        </p>
                      </div>
                    ) : (
                      <select
                        value={rollupLinkedColumnId}
                        onChange={(e) => setRollupLinkedColumnId(e.target.value)}
                        className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                      >
                        <option value="">Select a linked record column...</option>
                        {linkedRecordColumns.map((col) => (
                  ...
                  {rollupLinkedColumnId && (
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">
                        Step 2: Aggregation Function <span className="text-red-500">*</span>
                      </label>
                      <div className="grid grid-cols-3 gap-2">
                        {[
                          ...
                          <button
                            key={fn.value}
                            type="button"
                            onClick={() => setRollupFunction(fn.value)}
                            className={`p-3 border-2 rounded-lg text-left transition-all ${
                              rollupFunction === fn.value
                  ...
                  {rollupLinkedColumnId && rollupFunction && rollupFunction !== 'COUNT' && (
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">
                        Step 3: Select Field to Aggregate <span className="text-red-500">*</span>
                      </label>
                      {loadingRollupFieldColumns ? (
                        ...
                        >
                          <option value="">Select a field...</option>
                          {rollupLinkedTableColumns.map((col) => (
                            <option key={col.id} value={col.id}>
                              {getTypeIcon(col.type)} {col.name}
                            </option>
                          ))}
              ...
              {selectedType === 'linked_record' && (
                <div className="mb-6">
                  ...
                  {loadingTables ? (
                    ...
                    <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                      <p className="text-sm text-yellow-800">
                        No other tables available. Create another table first to link records.
                      </p>
                    </div>
                  ) : (
                    <select
                  ...
                  <p className="mt-2 text-xs text-gray-500">
                    💡 This column will link to records in the selected table
                  </p>
                  {linkedTableId && (
                    <div className="mt-4">
                      <label className="block text-sm font-medium text-gray-700 mb-2">
                        Display Column <span className="text-red-500">*</span>
                      </label>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {FORMULA_FORMATS.map((format) => (
                        <button
                          key={format.value}
                          type="button"
                          onClick={() => setFormulaFormat(format.value)}
                          className={`flex items-start p-3 border-2 rounded-lg text-left transition-all ${
                            formulaFormat === format.value
                              ? 'border-blue-500 bg-blue-50'
                              : 'border-gray-200 hover:border-gray-300'
                          }`}
                        >
            ...
            <div className="px-6 py-4 border-t border-gray-200 flex items-center justify-end space-x-3">
              ...
              <button
                ...
                disabled={
                  isCreating ||
                  !columnName.trim() ||
                  (selectedType === 'formula' && !formula.trim()) ||
                  (selectedType === 'linked_record' && !linkedTableId) ||
                  (selectedType === 'lookup' && (!lookupLinkedColumnId || !lookupFieldId)) ||
                  (selectedType === 'rollup' && (!rollupLinkedColumnId || !rollupFunction || (rollupFunction !== 'COUNT' && !rollupFieldId)))
                }
