      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col">
        ...
        <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between flex-shrink-0">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">Connect external APIs and sync data into this table</p>
          </div>
          ...
          {!creating ? (
            ...
            <div>
              <button onClick={startCreate}
                className="w-full mb-4 flex items-center justify-center space-x-2 px-4 py-3 border-2 border-dashed border-gray-300 rounded-lg text-gray-600 hover:border-blue-400 hover:text-blue-600 transition-colors">
              ...
              {loading ? (
                ...
                <div className="text-center py-12">
                  ...
                  <h3 className="text-sm font-medium text-gray-900 mb-1">No connectors yet</h3>
                  <p className="text-xs text-gray-500">Connect a REST API to sync external data into this table.</p>
                </div>
              ) : (
                <div className="space-y-3">
                  {connectors.map(function(c) {
                    var statusColor = c.lastSyncStatus === 'success' ? 'text-green-600' :
                      ...
                      c.lastSyncStatus === 'syncing' ? 'text-blue-600' : 'text-gray-400';
                      ...
                      <div key={c.id} className="border border-gray-200 rounded-lg p-4">
                        <div className="flex items-center justify-between mb-2">
                          <div className="flex items-center space-x-2">
                            ...
                            <div>
                              ...
                              <p className="text-[10px] text-gray-400">{(c.config as any)?.url || 'No URL'}</p>
                            </div>
                          </div>
                          <div className="flex items-center space-x-2">
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center space-x-3 text-[10px] text-gray-400">
                            ...
                            <span className="bg-gray-100 px-1.5 py-0.5 rounded">{c.syncMode === 'interval' ? 'Every ' + c.syncIntervalMin + 'min' : 'Manual'}</span>
                          </div>
                          <div className="flex items-center space-x-2">
                            ...
                            <button onClick={function() { startEdit(c); }}
                              className="px-2.5 py-1 text-[10px] text-gray-600 border border-gray-200 rounded hover:bg-gray-50">Edit</button>
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                <div className="flex space-x-2">
                  <select value={method} onChange={function(e) { setMethod(e.target.value); }}
                    className="w-24 px-2 py-2 border border-gray-300 rounded-lg text-sm bg-white text-gray-900">
                    <option value="GET">GET</option>
                    <option value="POST">POST</option>
                  </select>
                  <input type="text" value={url} onChange={function(e) { setUrl(e.target.value); }}
                    placeholder="https://api.example.com/data"
                    className="flex-1 px-3 py-2 border border-gray-300 rounded-lg text-sm font-mono focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white text-gray-900" />
                </div>
              </div>
                ...
                {authType === 'api_key' && (
                  <div className="flex space-x-2">
                    <input type="text" value={authHeader} onChange={function(e) { setAuthHeader(e.target.value); }}
                      placeholder="Header name" className="w-40 px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white text-gray-900" />
                    <input type="password" value={authValue} onChange={function(e) { setAuthValue(e.target.value); }}
                      placeholder="API key value..." className="flex-1 px-3 py-2 border border-gray-300 rounded-lg text-sm font-mono bg-white text-gray-900" />
                  </div>
                )}
                ...
                {authType === 'basic' && (
                  <div className="flex space-x-2">
                    <input type="text" value={username} onChange={function(e) { setUsername(e.target.value); }}
                      placeholder="Username" className="flex-1 px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white text-gray-900" />
                    <input type="password" value={password} onChange={function(e) { setPassword(e.target.value); }}
                      placeholder="Password" className="flex-1 px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white text-gray-900" />
                  </div>
                )}
              </div>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  Response Data Path <span className="text-gray-400 font-normal">(optional)</span>
                </label>
                <input type="text" value={responseDataPath} onChange={function(e) { setResponseDataPath(e.target.value); }}
                  ...
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm font-mono focus:ring-2 focus:ring-blue-500 bg-white text-gray-900" />
                <p className="text-[10px] text-gray-400 mt-1">Dot notation path to the array of records in the JSON response. Leave blank if the response IS the array.</p>
              </div>
                ...
                <div className="bg-blue-50 rounded-lg p-4 space-y-3">
                  ...
                  <div className="space-y-2 max-h-60 overflow-y-auto">
                    {previewFields.map(function(field) {
                      ...
                      return (
                        <div key={field} className="flex items-center space-x-2 bg-white rounded px-3 py-2">
                          <div className="flex-1 min-w-0">
                            <span className="text-xs font-mono text-gray-800">{field}</span>
                            {displayVal && <span className="text-[10px] text-gray-400 ml-2">"{displayVal}"</span>}
                          </div>
                          <span className="text-gray-300 text-xs">→</span>
                          <select value={fieldMapping[field] || ''}
                            onChange={function(e) { updateFieldMapping(field, e.target.value); }}
                            className="w-44 px-2 py-1.5 border border-blue-200 rounded text-xs bg-white text-gray-900">
                  ...
                  <div>
                    ...
                    <select value={uniqueKeyField} onChange={function(e) { setUniqueKeyField(e.target.value); }}
                      className="w-full px-3 py-2 border border-blue-200 rounded-lg text-xs bg-white text-gray-900">
              ...
              {testResult?.sample && testResult.sample.length > 0 && Object.keys(fieldMapping).length > 0 && (
                <div className="bg-gray-50 rounded-lg p-4">
                  <label className="block text-xs font-bold text-gray-500 mb-2 uppercase tracking-wider">Preview — First Record</label>
                    ...
                    {Object.entries(fieldMapping).map(function(entry) {
                      ...
                      return (
                        <div key={apiField} className="flex items-center text-xs">
                          <span className="w-32 text-gray-500 truncate">{col?.name || colId}</span>
                          <span className="text-gray-300 mx-2">←</span>
                          <span className="text-gray-800 font-mono">{val !== undefined ? String(val) : '(empty)'}</span>
                        </div>
                      );
                    })}
                  </div>
