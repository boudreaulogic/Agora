  ...
  return (
    <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '32px 24px' }}>
      <div style={{ marginBottom: '24px' }}>
        <h1 style={{ fontSize: '22px', fontWeight: 700, color: '#111827', margin: 0 }}>🛡️ Security Monitoring</h1>
        <p style={{ fontSize: '13px', color: '#6b7280', marginTop: '4px' }}>Login activity, MFA management, and account security</p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
        ...
        <div style={{ background: '#ffffff', border: '1px solid #e5e7eb', borderRadius: '10px', padding: '16px' }}>
          <div style={{ fontSize: '13px', fontWeight: 600, color: '#111827', marginBottom: '12px' }}>User Security Status</div>
          <div style={{ maxHeight: '400px', overflowY: 'auto' }}>
            {data.users.map(function(user: any) {
              var isLocked = user.lockedUntil && new Date(user.lockedUntil) > new Date();
              return (
                <div key={user.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px', borderBottom: '1px solid #f3f4f6', fontSize: '12px' }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontWeight: 500, color: '#111827', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' as const }}>{user.name || user.email}</div>
        ...
        <div style={{ background: '#ffffff', border: '1px solid #e5e7eb', borderRadius: '10px', padding: '16px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
            <div style={{ fontSize: '13px', fontWeight: 600, color: '#111827' }}>Login Activity</div>
          ...
          <div style={{ maxHeight: '400px', overflowY: 'auto' }}>
            {filteredActivity.length === 0 ? (
              ...
              filteredActivity.map(function(log: any) {
                ...
                return (
                  <div key={log.id} style={{ display: 'flex', alignItems: 'flex-start', gap: '8px', padding: '6px 0', borderBottom: '1px solid #f9fafb', fontSize: '11px' }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <span style={{ color: '#374151' }}>{log.user?.name || log.user?.email || 'Unknown'}</span>
                      {meta.email && <span style={{ color: '#9ca3af', marginLeft: '4px' }}>({meta.email})</span>}
                      {meta.reason && <span style={{ color: '#dc2626', marginLeft: '4px' }}>— {meta.reason}</span>}
                    </div>
                    <span style={{ color: '#9ca3af', fontSize: '10px', whiteSpace: 'nowrap' as const, flexShrink: 0 }}>
